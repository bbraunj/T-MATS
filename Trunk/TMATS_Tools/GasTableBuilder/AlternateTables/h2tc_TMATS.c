...
#include "simstruc.h"
#include <math.h>
double h2tc(double Enthalpy, double FAR)
{
    double tg; 
    int interpErr = 0;
    double zi, yi, xi;
...
/* FAR */
yi = FAR;
while ((fabs(Enthalpy - P) >= Htol && iter < IterMax) || (iter < 2)){
...
if (iter >= IterMax)
printf("Warning in Enthalpy to Temperature. Correct temperature could not be found\n");
Tfin = xi;
return Tfin;
 }